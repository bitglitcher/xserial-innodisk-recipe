 ...
 * ---------------- Innodisk Memo ----------------
 * @ Before v2.3.1:
 *   - The driver we used is provided by Exar (native)
 *
 * @ v2.3.1:
 *   - Add MODULE_VERSION
 ...
 *
 * @ v2.3.2:
 *   - Mark off "Configuring MPIO as inputs" for 8 ports transceiver
 * 
 * @ v2.3.3:
 *   - Unmark "tty_struct in receive_chars" to support kernel before 3.9.0
 * 
 * @ v2.3.4:
 *   - support USE_PORT_NUM=2, 4
 * 
 * @ v2.6.1:
 *   - update to Exar driver v2.6
 */

#define INNO_MODULE_VERSION	"v2.6.1"
/* ----------------------------- */
...
/* ----- Innodisk modified ----- */
#ifndef _USE_PORT_NUM
	#define _USE_PORT_NUM	4
#endif
/* ----------------------------- */
...
static int __init serialxr_init(void)
{
	...
	printk(KERN_INFO "Exar PCIe (XR17V35x) serial driver Revision: %s\n", INNO_MODULE_VERSION);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("This kernel module is for the series of Innodisk EMP2 products - Based on Exar PCIe specific serial driver XR17V35x (vesion: 2.6)");
MODULE_VERSION(INNO_MODULE_VERSION);
